...
def prep_mlm(fn_ctx, fn_r):
  outputs = tokenize(open(fn_r).readlines())
  valid_src = [e.strip().split("_eos ")[-1] for e in open(fn_ctx).readlines()]
  output_lines = [s + " " + r + "\n" for s,r in zip(valid_src, outputs)]
  open("undr/" + 'test.lm', "w+").writelines([' '.join(e.split()) + "\n" for e in output_lines])

def prep_both(fn_ctx, fn_r):
  outputs = tokenize(open(fn_r).readlines())
  valid_src = [e.strip().split("_eos ")[-1] for e in open(fn_ctx).readlines()]
  valid_fct = [e.strip() for e in open(fn_ctx).readlines()] 
...
def get_scores(fn):
  prep_mlm('dialogue_context.txt', 'dialogue_response.txt')
  #prep_bs(fn)
  prep_both('dialogue_context.txt', 'dialogue_response.txt')
  prep_uk('dialogue_context.txt', 'dialogue_response.txt')
...
python3 train_understandable.py \
    --per_gpu_eval_batch_size=1 \
    --output_dir=/home/yiting/usr/pretrained_models/ctx \
    --model_type=roberta \
...
export DATA_DIR=fct/{0}/
    ...
    python3 train_understandable.py \
    --per_gpu_eval_batch_size=1 \
    --output_dir=/home/yiting/usr/pretrained_models/uk \
    --model_type=roberta \
    --model_name_or_path=roberta-base \
    --data_dir $DATA_DIR \
    --do_eval \
...
""".format(fn_base)
  ...
  fn_base = 'test'
  mlm = """
export EVAL_FILE=undr/{0}.lm

    python3 run_lm_finetuning.py \
    --per_gpu_eval_batch_size=1 \
    --output_dir=/home/yiting/usr/pretrained_models/roberta_ft \
    --model_type=roberta \
    --model_name_or_path=roberta-base \
